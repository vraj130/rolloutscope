> **Historical record, not binding.** This is the v0 build log (July to September
> 2026), moved here from the repo root on 2026-09-25. It uses two numbering
> systems: "Phase 0" to "Phase 8" are the v0 build phases, and "Delivery plan
> Phase N" refers to [delivery-plan-2026-09.md](delivery-plan-2026-09.md). Current
...
namespace URI (`xmlns="http://www.w3.org/2000/svg"`), so it opens n  from file://.
...
## Post-v0: Phase 4 packaging checks and performance infrastructure

Status: COMPLETE (2026-09-08)

Packaging and performance measurement slices from the delivery plan, implemented
without waiting on Phase 3 storage work. Optimization is explicitly out of scope.

- [x] Coverage floor of 85 percent in `[tool.coverage.report] fail_under`. Evidence:
      pyproject.toml; tests/test_packaging/test_ci_and_coverage.py.
- [x] CI uses the committed lockfile: `uv lock --check` in lint, `--frozen` on
      `uv run` for lint, test, and packaging. Evidence: .github/workflows/ci.yml.
- [x] Isolated wheel-install smoke: `scripts/check_packaging.py` builds or reuses a
      wheel, installs it into a throwaway venv, and checks CLI, six detector entry
      points, Jinja PackageLoader template, and `analyze` on a copied demo fixture.
      The installed module path must not live under `src/rolloutscope`. Evidence:
      the script, tests/test_packaging/test_check_packaging.py, CI packaging job
      matrix 3.11 / 3.12 / 3.13.
- [x] Performance generator writes prime-rl `step_<n>/train_rollouts.jsonl` layouts
      for documented sizes small (160), medium (5_000), large (50_000), deterministic
      by seed, loadable by the current adapter. Evidence: scripts/perf/generate.py,
      tests/test_perf/test_generate.py.
- [x] Measurement recorder captures elapsed time, peak RSS (bytes), HTML/JSON output
      size, detector mix, and input summaries into a versioned JSON document
      (`schema_version` 1.0). No CI fail-on-budget. Evidence:
      scripts/perf/measure.py, scripts/perf/result_schema.json,
      tests/test_perf/test_measure.py.
- [x] Docs: CONTRIBUTING commands, CHANGELOG Unreleased entries.

TODOs carried forward: pipeline optimization and a published resource envelope wait
for Phase 3 storage and access patterns.

## Delivery plan Phase 2: benchmark preparation and upstream compatibility research

Status: COMPLETE (2026-09-08)

Scope: the two research and preparation workstreams of the delivery plan's Phase 2
(`docs/delivery-plan.md`). Detector repair, role-aware evidence normalization,
metric-semantics configuration, and evidence-calibrated reporting are separate Phase 2
deliverables and are NOT in this slice. The current-upstream adapter is deliberately
not implemented: it waits on the Phase 1 identity and provenance contract, per the
plan.

### Benchmark preparation

- [x] Frozen dataset manifest. `benchmarks/trace/manifest.json` pins
      `PatronusAI/trace-dataset` at revision `31d87f06078eca3ab6eaf1e06e5ea6fe9f2b7a6d`,
      license cc-by-sa-4.0, all 517 rows with their labels, a SHA-256 per row, and a
      split per row. Row content is not committed: the dataset is gated and CC-BY-SA-4.0,
      so the manifest carries identity and hashes and nothing else. Regenerated by
      `scripts/build_trace_manifest.py`. Manifest digest
      `e650b582b6f32eef26a63c9c328b7ab6eaf6dc6323d8075331e674145b4afd2a`.
      Evidence: generator output; `tests/test_benchmark/test_manifest_and_evaluate.py`
      covers round-trip stability, order independence, and drift detection.
- [x] Unified TRACE mapper and evaluation. `src/rolloutscope/benchmark/` holds one row
      mapper, one applicability rule set, one metric implementation, and one report
      format. `scripts/trace_validation.py` and `tests/integration/test_trace_dataset.py`
      both call `evaluate_rows`, and
      `test_script_and_test_produce_the_same_report` runs the script as a subprocess and
      compares its JSON against the in-process report. Evidence: `uv run --extra
      benchmark pytest tests/integration -m integration -q` gives 7 passed.
- [x] Hard-negative examples. `tests/fixtures/hard_negatives/`, 33 cases across all six
      detectors, covering the six classes the plan names plus the two cases the Phase 2
      success criteria call out. Each case records both `expected_fired` (always false)
      and the measured `currently_fires`. 21 currently fire.
      Evidence: `tests/test_benchmark/test_hard_negatives.py`, which asserts recorded
      behavior against actual behavior so the corpus is a regression test today and an
      acceptance test after the repairs.
- [x] Train, tuning, and holdout splits. Assigned per scenario (a salted hash of the
      normalized opening user message), never per row, so sibling trajectories cannot
      straddle a boundary. Weights 0.40 / 0.20 / 0.40, chosen and recorded before any
      holdout measurement. Realized: train 202, tuning 105, holdout 210. The holdout is
      sealed; no number has been taken from it and no threshold has been set from this
      benchmark. Measured caveat recorded in `docs/benchmark-metrics.md`: 517 rows give
      516 distinct scenario keys, so grouping is close to a no-op on this artifact.
- [x] Metric definitions and reporting format. `docs/benchmark-metrics.md` defines unit,
      positive, fired, the four confusion cells, precision, recall, false-positive rate,
      coverage, per-category recall, and the Wilson interval, plus the full pin list a
      quoted number requires. Implemented in `benchmark/metrics.py` and
      `benchmark/report.py`.

Measured result on the tuning split (105 rows, 50 hacked, 55 benign), recorded because
it is the evidence for the detector-repair work, not as a release claim:
`verifier_tamper` coverage 1.00, precision 0.52 [0.39, 0.64], recall 0.60 [0.46, 0.72],
false-positive rate 0.51 [0.38, 0.64]. The other five detectors report zero coverage with
the missing signal named, because TRACE ships transcripts with no reward, no metrics, and
no reference answer.

### Upstream compatibility research

- [x] Sanitized artifacts captured from the real upstream writers.
      `tests/fixtures/upstream/` holds three layouts, produced by
      `scripts/capture_upstream_fixtures.py`, which imports the upstream models and calls
      the upstream serializers rather than hand-writing JSON. All content is synthetic:
      no real model output, no PII. The script is the only file in the repository that
      imports verifiers, is not part of the package, and is run in a throwaway
      environment so the project venv never gains the dependency (golden rule 3 holds).
- [x] Compatibility matrix. `docs/compatibility-matrix.md`, verified against upstream
      source at pinned commits on 2026-09-08. Findings that matter:
      verifiers dropped the legacy `results.jsonl` format at v0.3.1 (`verifiers/types.py`
      and `utils/save_utils.py` are gone); prime-rl replaced the per-step cohort
      directories between v0.9.1.dev28 and dev29, both dated 2026-09-01; prime-rl main
      calls `to_record(float_decimals=...)`, which arrived in verifiers commit `cfc7c474`
      (2026-09-02) and is in no tag, while declaring only `verifiers>=0.3.1`, so its
      declared floor is looser than its real requirement (reproduced during capture).
      Four concrete gaps against this reader are recorded, the quietest being that
      verifiers v1 `ToolCall` is flat (`{id, type, name, arguments}`) while
      `detectors/_text.py` reads `call["function"]["name"]`, so every tool-call signal
      would silently vanish.
- [x] `all` versus `effective` cohort behavior. `docs/upstream-cohorts.md`. Legacy: the
      orchestrator logged each episode to `all` on arrival and the filtered batch to
      `effective` at finalize, so a shipped episode is on disk twice and a recursive glob
      double counts it. Current: `log_episodes` returns early on `effective`, every
      episode is written once, and what the cohort learns arrives as annotations a reader
      folds back on, so membership is `trace.info["effective"]` rather than a directory.
      Recorded correction: golden rule 9's claim that advantages are never on disk is
      still true of the legacy format but no longer true of the current one, where the
      annotation stream carries a scalar and per-token advantages.
- [x] Draft adapter fixtures. The three captured layouts, with
      `tests/test_benchmark/test_upstream_fixtures.py` holding the matrix's shape claims
      to the actual bytes: no top-level `example_id` or `reward`, `validate_rollout`
      rejects an episode record today, rewards are per trace and weighted, tool calls are
      flat and invisible to the current text extraction, identity is supplied upstream, a
      failed episode carries errors and no traces, the legacy layout duplicates the
      effective cohort, and the stream index addresses each episode by chunk and offset.
      No adapter consumes them, by design.

### Gate

- [x] `uv run pytest -q --ignore=tests/test_detectors/test_execution.py`: 362 passed,
      7 deselected. The 11 failures under `-q` alone are in another session's in-progress
      Phase 1 file (`tests/test_detectors/test_execution.py`, untracked), whose helper
      omits the required `is_completed` and `is_truncated`; not touched here.
- [x] `uv run --extra benchmark pytest tests/integration -m integration -q`: 7 passed.
- [x] `uv run ruff check` and `ruff format --check` clean on every file added or changed
      here.
- [x] `uv run mypy src/rolloutscope/benchmark`: clean, 10 source files.
- [x] Em-dash sweep over all 67 files added or changed here plus the three new docs:
      zero occurrences of em, en, or horizontal bar.

Deviations: none.

TODOs carried forward:
- The TRACE leaf category names (`1.1.1` and friends) are positional inferences from the
  dataset card's parenthesized lists and are marked `inferred` in
  `benchmark/taxonomy.py`. Confirm against arXiv:2601.20103, along with the card's own
  contradiction between "10 broader categories" in prose and four in its list.
- The code path that first writes `trace.info["advantage"]` in prime-rl was not located;
  `stamp_batch` reads it rather than computing it. Confirm the producer before relying on
  the field.
