...
class DegenerateRepetitionDetector:
    ...
    def detect(self, rollouts: Sequence[Rollout], config: DetectorConfig) -> list[Verdict]:
        ...
        ``config.degenerate_repetition``). Rollouts below ``min_reward`` or
        with fewer than ``min_tokens`` word tokens are skipped. The evidence
        span carries the most-repeated n-gram, located in the raw completion
        text when possible.
        """
